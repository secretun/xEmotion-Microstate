% pop_micro_segment() - select settings for segmentation into microstates  选择设置细分为微状态
%
% Function for segmenting EEG into microstates using one of several
% selectable clustering methods: 使用几种可选择的聚类方法之一将脑电图分割为微状态的功能: 
...
%   EEG - Input dataset.
%
% Optional inputs: 可选的输入
%  'algorithm'          - String denoting the algorithm used for 表示分割算法的字符串
...
%  'Nmicrostates'       - Number of microstates to segment EEG into. If  脑电分割的微状态数。
%                         vector then each value will be tested and the
%                         optimum number of microstates selected based on
%                         measures of fit (default: 3:8). 如果向量，则每个值将被测试，并根据拟合措施选择最佳微状态数(默认:3:8)。
%  'sorting'            - Method of sorting microstates. {'Global explained  微观状态的分选方法。
%                         variance','Chronological appearance','Frequency'}
%                         (default: 'Global explained variance').
%  'verbose'            - Print status messages to command window? 将状态消息打印到命令窗口
%                         (default:1).
...
%
% Outputs:
%   EEG    - Output dataset. This function saves output in the substruct  输出数据集。 这个函数将输出保存在子结构OUTEEG中。
%            OUTEEG.microstate, which contains general info and algorithm-
%            specific settings as well as the results of segmentation. The  微状态，其中包含一般信息和算法特定的设置以及分割的结果。
%            .Res substruct contains algorithm specific results. See the
%            relevant algorithm function for explanations on variable
%            names.  Res子结构包含算法特定的结果。 有关变量名的解释，请参阅相关算法函数。  
%
%
...
%% Get data for segmentation 获取分割数据
...
switch settings.algorithm
    ...
    case 'varmicro'
        ...
        opts_settings_names = {
            ...
            'thresh' 'threshold';
            'verbose' 'verbose'};
        opts = algosettings_to_opts(settings,opts_settings_names);  % 位置于 867-877
        K_range = settings.algorithm_settings.Nmicrostates;
    ...
    case 'kmeans'
        settings.temporal_smoothing = 0; % no post-segment smoothing 没有post-segment平滑
        % starting K-merans using subfunction 使用子函数启动K-merans
        EEG = run_kmeans(data,EEG,settings);  % 位置于879-951
...
EEG = sort_microstates(data,EEG,sort_names_opt);  % 位置在1049-1153


%% Define command string
com = sprintf('%s = pop_micro_segment( %s', inputname(1), inputname(1));
com = settings_to_string(com,settings);  % 位置1171-1204
com = [com ' );'];
...
if isstruct(pop_out)
    settings = struct;
    settings = interpret_popup(pop_out, settings, style, popmenu);  % 位置于 777-799
...
if ~strcmp(settings,'cancel')
    switch settings.algorithm
        case 'modkmeans'
            settings = modk_popup(settings);  % 位置于 429-516
        case 'kmeans'
            settings = kmeans_popup(settings);  % 位置于 641-712
        case 'taahc'
            settings = taahc_popup(settings);  % 位置于714-773
        case 'Experimental algorithms'
            settings = experimental_popup(settings);  % 位置于518-561
        otherwise
            error(['selected algorithm,''' settings.algorithm ''' not available'])
    end
end
...
% Arranges and renames relevant algorithm settings into opts struct.  安排和重命名相关的算法设置到opts结构。
% "names" is a Nx2 cell, where each row is a opts.name1 = settings.name2
% pair. Does not carry empty fields over  “names”是一个Nx2单元格，其中每一行是一个
...
for K = K_range
    ...
    if settings.temporal_smoothing
        L = smoothing(X,A,L,K,settings);  % 位置于973-1047
    end
    ...
    % Calculating 
    [MSE(K_ind), MSE_mcv(K_ind)] = calc_MSE(X,A,L,K);  % 位置于 953-971
...
for K = K_range
    ...
    switch EEG.microstate.sorting
        case 'Global explained variance'
            ...
            C2 = columncorr(X,A(:,L));  % 位置于1155-1169
            GEV = (GFP.*C2).^2;
            sortmeas = zeros(K,1);
            for k=1:K
