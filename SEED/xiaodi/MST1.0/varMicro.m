...
%  segments as described in [1]. 实现变分微状态，可选的平滑段，如[1]所述。  
...
%  number of microstates. 自由能用于在相同的微观状态数(K)中进行多次重启的选择。自由能也用于寻找最佳的微观状态数。
%
...
%
%  Outputs
%  A_opt   - Spatial distribution of microstates (channels x K). 微态的空间分布(通道x K)  
%  L_opt   - Label of the most active microstate at each timepoint (1 xsamples).  每个时间点最活跃微状态的标签(1 x样本)。
%  Res     - Struct containing additional results: 包含额外结果的结构
...
for K = K_range
    ...
    % Finding optimum solution amongst different values of K 在K的不同值中寻找最优解  
