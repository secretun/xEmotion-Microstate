...
clear all
%% 读取数据整合
[feature_data, txt, raw2] = xlsread('E:\SEED\SEED_EEG\分开导入\实验融合\8.xlsx',5,'A1:AO480');
data = feature_data(:,2:41);
%data = feature_data(:,2:201);
% lable = feature_data(:,1);
data(isnan(data)) = 0;
...
model = svmtrain(newtrainY, newtrainX, cmd );
...
% 
% % 素贝叶斯分类器
% nb = fitcnb(newtrainX, newtrainY);
% % 预测测试集标签
% nbPredictions = predict(nb, newtestX);
% % 评估分类器性能
% confMat = confusionmat(newtestY, nbPredictions);
% accuracyb = sum(diag(confMat)) / sum(confMat(:));
% accuracy_bys(i,:) = accuracyb;
end
mean_accuracys =mean(accuracy_svm(:,1));
svm_sd=std(accuracy_svm(:,1));
mean_accuracyk = mean(accuracy_knn);
knn_sd=std(accuracy_knn);
%mean_accuracyb =mean(accuracy_bys);