% [] = pop_micro_selectNmicro() - Select active number of microstates. 选择微状态的活跃数。
%
...
%  each number of microstates.   选择微状态的活跃数。 默认情况下，该函数将绘制一个新的图形，每个微状态数的最佳分割的四个措施。
%  The user can also directly define the active number of microstates
%  without plotting measures.  用户还可以直接定义微状态的活跃数量，而无需绘制度量
%  Note: Use the function MicroPlotFitmeas to plot measures of fit in an
%  existing figure.  使用MicroPlotFitmeas函数在现有的图形中绘制适合的度量
%
% Usage:
...
%   输出数据集。 这个函数将输出保存在子结构OUTEEG中。 微状态，其中包含信息一般和算法特定的设置以及分割的结果。  
% Authors:
%
...
if nargin < 2
    % pop-up window in case no further input is given  弹出窗口，以防没有进一步输入  
    settings = input_popup();   % 位置于144-248
    if strcmp(settings,'cancel')
        return
    end
else
    settings = check_settings(varargin);  % 位置于340-350
end;

Nmicro = settings.Nmicro;


%% Check and ordering of selected measures of fit 对选定的配合措施进行检查和排序 
...
% check if measures of fit are present  检查是否存在合适的度量
...
% 弹出，用于绘制拟合措施和选择微状态的活跃数量  
if isempty(Nmicro)
    Nmicro = fitmeas_popup(EEG, Measures, settings);  % 位置于 250-313
...
%% Interpret output from popup 解释弹出的输出
if isstruct(pop_out)
    settings = struct;
    settings = interpret_popup(pop_out, settings, style);  %位置于 316-338
    
    % Save selected measures of fit in Measures 在措施中保存适合的选定措施
...
% select number of microstates. 创建一个带有plot(多个)的弹出框用于匹配度量，以及一个用于选择微状态数量的按钮。
...
% 找到plot_idx并检查plot_range中的所有元素都处于n微状态
