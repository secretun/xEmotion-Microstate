  ...
  for round = 1:maxNumIterations + 1,
    ...
    switch usedNlinearity
     ...
     case 11
      ...
      % music_m2 ok
      %Gpow3 = (X' * B) .^ 3;
      %Beta = ones(size(B)) * B' * X * Gpow3;
      %B = B - myy * (X * Gpow3 - Beta .* B) ./ (3 * numSamples - Beta);
      
      %Y = B' * X;
      %Beta = sum((Y .* (Y .^ 3))') / numSamples;
      %D = diag(1 ./ (Beta - sum((3 * Y .^2)') / numSamples));
      %%D = diag(1 ./ (Beta - 3));
      %B = B + myy * B * (Y * (Y' .^ 3) / numSamples - diag(Beta)) * D;
     ...
     case 13
      ...
      %Xsub=X(:, getSamples(numSamples, sampleSize));
      %Gpow3 = (Xsub' * B) .^ 3;
      %Beta = ones(size(B)) * B' * Xsub * Gpow3;
      %B = B - myy * (Xsub * Gpow3 - Beta .* B) ./ ...
      %	  (3 * size(Xsub,2) - Beta);
     ...
     case 20
	  ...
	  a1;
     case 21
      % Tämä oli vanha koodi
      %      hypTan = tanh(a1 * X' * B);
      %      Beta = ones(size(B)) * B' * X * hypTan; 
      %      B = B - myy * ((X * hypTan - Beta .* B) ./ ...
      %		     (ones(size(B,1),1) * a1 * sum(1 - hypTan .^ 2) - ...
      %		      Beta));

      % Tämä ja seuraava antaa identtisen vastauksen, ja toimii demosig-
      % datalla ok verrattuna 20:een.
      % Mutta mixmus-datalla vain kaksi komponenttia oli ok verrattuna 20:een.

      %Y = B' * X;
      %Beta = sum((Y .* tanh(Y))') / numSamples;
      %D = diag(1 ./ (Beta - sum((1 - tanh(Y) .^ 2)') / numSamples));
      %B = B + myy * B * (Y * tanh(Y') / numSamples - diag(Beta)) * D;
      ...
      %Y = X' * B;
      %Beta = sum(Y .* tanh(a1 * Y)) / numSamples;
      %D = diag(1 ./ (Beta - a1 * sum(1 - tanh(a1 * Y) .^ 2) / numSamples));
      %B = B + myy * B * (Y' * tanh(a1 * Y) / numSamples - diag(Beta)) * D;
     ...
     case 23
      ...
      %Xsub=X(:, getSamples(numSamples, sampleSize));
      %hypTan = tanh(a1 * Xsub' * B);
      %Beta = ones(size(B)) * B' * Xsub * hypTan;
      %B = B - myy * ((Xsub * hypTan - Beta .* B) ./ ...
      %(ones(size(B,1),1) * a1 * sum(1 - hypTan .^ 2) - Beta));
     ...
     case 31
      % Vanha
      % demosig ok
      % (vain 2 oikein mixmus)
      %U = X' * B;
      %Usquared=U .^ 2;
      %ex = exp(-a2 * Usquared / 2);
      %gauss =  U .* ex;
      %dGauss = (1 - a2 * Usquared) .*ex;
      %Beta = ones(size(B)) * B' * X * gauss;
      %B = B - myy * (X * gauss - Beta .* B) ./ ...
      %	  (ones(size(B,1),1) * sum(dGauss) - Beta);
      
      %Y = B' * X;
      %ex = exp(-a2 * (Y' .^ 2) / 2);
      %gauss = ex' .* Y;
      %dGauss = (1 - a2 * (Y .^ 2)) .* ex';
      %Beta = sum((Y .* gauss)') / numSamples;
      %D = diag(1 ./ (Beta - sum(dGauss') / numSamples));
      %B = B + myy * B * (Y * gauss' / numSamples - diag(Beta)) * D;
     ...
     case 33
      ...
      %gauss =  U .* ex;
      %dGauss = (1 - a2 * Usquared) .*ex;
      %Beta = ones(size(B)) * B' * Xsub * gauss;
      %B = B - myy * (Xsub * gauss - Beta .* B) ./ ...
      %  (ones(size(B,1),1) * sum(dGauss) - Beta);
     ...
     case 41
      % Vanha
      %Gskew = (X' * B) .^ 2;
      %Beta = ones(size(B)) * B' * X * Gskew / numSamples;
      %B = B - myy * ((X * Gskew / numSamples) - Beta .* B) ./ (-Beta);
      
      %Y = B' * X;
      %Beta = sum((Y .* (Y .^ 2))') / numSamples;
      %%D = diag(1 ./ (Beta - sum((2 * Y)') / numSamples));
      %%sum((2 * Y)') / numSamples
      %D = diag(1 ./ (Beta));
      %B = B + myy * B * (Y * (Y .^ 2)' / numSamples - diag(Beta)) * D;
     ...
     case 43
      ...
      %Xsub=X(:,getSamples(numSamples, sampleSize));
      %Gskew = (Xsub' * B) .^ 2;
      %Beta = ones(size(B)) * B' * Xsub * Gskew / size(Xsub,2);
      %B = B - myy * ((Xsub * Gskew / size(Xsub,2)) - Beta .* B) ./ (-Beta);
