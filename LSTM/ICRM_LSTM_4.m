...
%             C_postica = mean_covariances(train_cov_postica);
            ...
            Strain = (Strain - mean(Strain,'all')) / std(Strain,0,'all');
            Stest = (Stest - mean(Stest,'all')) / std(Stest,0,'all');
            ...
            % 测试 LSTM 网络
            % 对测试数据进行分类
            YPred_2 = classify(net_2,XTest, ...
                'MiniBatchSize',miniBatchSize, ...
                'SequenceLength','longest');
            % 计算预测值的分类准确度
            acc_2(k) = sum(YPred_2 == YTest)./numel(YTest)
...
%     ACC.icrm_2lstm = ACC_postica_2;
    ...
    save('G:\MAHNOB\MATLAB程序\ACC_noise_noica','-struct','ACC');
