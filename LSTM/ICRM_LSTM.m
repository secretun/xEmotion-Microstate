...
%                 dropoutLayer(0.5)
            ...
            YPred = classify(net,XTest, ...
                ...
                'SequenceLength','longest');
            % 计算预测值的分类准确度
            acc(k) = sum(YPred == YTest)./numel(YTest)
...
%                 dropoutLayer(0.3)
                ...
                softmaxLayer
                classificationLayer];
            net_prerm = trainNetwork(XTrain_prerm,YTrain,layers,options);
            % 测试 LSTM 网络
            % 对测试数据进行分类
            YPred_prerm = classify(net_prerm,XTest_prerm, ...
                'MiniBatchSize',miniBatchSize, ...
                'SequenceLength','longest');
            % 计算预测值的分类准确度
            %
            acc_prerm(k) = sum(YPred_prerm == YTest)./numel(YTest)
...
%                 dropoutLayer(0.5)
                ...
                softmaxLayer
                classificationLayer];
            net_postica = trainNetwork(XTrain_postica,YTrain,layers,options);
            % 测试 LSTM 网络
            % 对测试数据进行分类
            YPred_postica = classify(net_postica,XTest_postica, ...
                'MiniBatchSize',miniBatchSize, ...
                'SequenceLength','longest');
            % 计算预测值的分类准确度
            %
            acc_postica(k) = sum(YPred_postica == YTest)./numel(YTest)
        ...
        ACC(sub_no,i) = mean(acc);
        ACC_prerm(sub_no,i) = mean(acc_prerm);
        ACC_postica(sub_no,i) = mean(acc_postica);
        clear samp samp_postica acc acc_postica
    end
end