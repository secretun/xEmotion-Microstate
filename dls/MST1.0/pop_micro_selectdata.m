% pop_micro_selectdata() - selects and aggregates data for microstate segmentation.  为微状态分割选择和聚合数据  
%
% pop_micro_selectdata selects data for microstate segmentation, in
% severeral different ways:  以几种不同的方式为微状态分割选择数据:
% * For spontaneous data: selection of GFP peaks.  自发数据:GFP峰的选择
...
% ALLEEG.  可以从ALLEEG中加载的多个数据集聚合数据。 当聚合数据时，一个新的数据集'NewEEG'将被创建并添加到ALLEEG。  
%
% Usage:
...
%                      EEGlab as EEG structures.  EEGlab结构包含所有作为EEG结构读入EEGlab的数据集
...
%                      across subjects.  进入分割的每个受试者的GFP峰值数。 请注意，最大的峰值数量被限制在GFP峰值的最小数量。  
...
%                      (default). 拒绝峰值超过阈值(std(GFP)的倍数)。 设置为避免可能由人为因素造成的极端GFP峰值。 设置为零以关闭(默认)  
% Outputs:
%   EEG        - EEGlab EEG structure with added microstate field. 增加微状态场的脑电结构  
%   ALLEEG     - EEGlab structure containing all datasets read into
%                EEGlab as EEG structures.  EEGlab结构包含所有数据集读取到EEGlab作为脑电图结构  
%   CURRENTSET - Workspace variable index of the current dataset. Only  当前数据集的工作区变量索引  
%                relevant when creating a new dataset through aggragating
%                data.  仅在通过聚合数据创建新数据集时相关
%   NewEEG     - New EEGlab EEG structure containing data selected for
%                microstate analysis and info in added microstate field. 新的EEGlab脑电结构包含了微状态分析数据和附加的微状态场信息。 
%
% Authors:
...
%% do the aggregation for spontaneous or erp data 是自发数据还是erp数据的聚合  
if strcmp(settings.datatype,'spontaneous')
    %% Readying settings for countinuous data 
    Npeaks = settings.Npeaks;
    MinPeakDist  = settings.MinPeakDist;   % 设置的值为10
    ...
    for i=1:Ndatasets
        ...
        %% Parse GFP peaks 解析GFP的峰值
        % do the segmentation on the GFP peaks, because they are high in
        % signal-to-noise  GFP峰上的分割，因为它们的信噪比很高  
        
        % calculate the GFP
        GFP = double(std(X,[],1));
        
        % Minimum distance in ms:
        MinTFdistance = MinPeakDist * fs/1000; % assuming ms 假设ms    MinPeakDist = 10
    ...
    %% Select the GPF peaks  选择GPF峰值
    % select a number of random peaks (if there are many peaks or you are
    % impatient)  选择一些随机的峰值(如果有很多峰值或者你很不耐烦) 
    % this is the maximum number that can be selected of each subject    这是每个被试可以选择的最大数量  
    ...
    for i = 1:Ndatasets
        ...
        % find a number of random peaks  找到一些随机的峰
    ...
    %% Save chosen data in the EEG struct  将选定的数据保存到脑电图结构中
    if aggregate_data
        % Add information to NewEEG struct  向NewEEG结构体添加信息
