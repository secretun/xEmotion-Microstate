...
%  prototypes numbers.  使用地形图为一系列微观状态原型编号绘制一个图表  
%
%  Usage:
...
%             the field 'chanlocs'; and 'prototypes' fields in  脑电图-实验室脑电图结构(通道x个样本(x个时代))与场的“非混沌”;
%             EEG.microstates.   以及脑电图微观状态下的“原型”字段。  
%
%  Optional input:
%  'plot_range'  - The range of numbers of microstates to be displayed (as
%                  vector). If empty (default), measures for all microstate
%                  numbers will be plotted.  要显示的微观状态数的范围(作为向量)。 如果为空(默认值)，则绘制所有微观状态数的度量 
%
% Author:
...
MAX_TOPO_WIDTH = 16; % maximum number of microstates to be plotted 要绘制的最大微观状态数

% check if chanlocs are present 检查是否有异常
...
%% Compute title and axes font sizes 计算标题和轴的字体大小
% figure('Name','Microstates');
pos = get(gca,'Position');
% pos = [0.1300 0.1100 0.7750 0.8150];
axis('off') % don't show axis 不显示轴
cla % clear the current axes 清楚当前轴
if pos(4)>0.70
    axfont = 16;
...
%% Compute topoplot head width and separation 计算地形图头的宽度和间隔
head_sep_x = 0.1; % x-axis distance between topos as a ratio of topo_width  topos之间的x轴距离与topo_width的比值  
head_sep_y = 0.3; % y-axis distance between topos as a ratio of topo_width
ylabel_space = pos(3)/(K_max*3); % space for y axis labels  y轴标签的空间
% calculate width/height of topoplots  计算地形的宽度/高度
...
% Loop over rows and collumns 循环遍历行和列
for r = 1:Nrows
